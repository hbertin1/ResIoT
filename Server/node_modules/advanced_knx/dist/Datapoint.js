...
Datapoint.prototype.bind = function (conn) {
  ...
  this.conn = conn; // bind generic event handler for our group address

  let gaevent = util.format('event_%s', self.options.ga);
  conn.on(gaevent, function (evt, src, buf) {
    let jsvalue = buf; // get the Javascript value from the raw buffer, if the DPT defines fromBuffer()
...
Datapoint.prototype.write = function (value) {
  ...
  if (this.dpt.hasOwnProperty('range')) {
    ...
    if (value < range[0] || value > range[1]) {
      throw util.format('Value %j(%s) out of bounds(%j) for %s', value, typeof value, range, this.dptid);
    }
  }
...
Datapoint.prototype.toString = function () {
  return util.format('(%s) %s %s', this.options.ga, this.current_value, this.dpt.subtype && this.dpt.subtype.unit ? this.dpt.subtype : '');
};
