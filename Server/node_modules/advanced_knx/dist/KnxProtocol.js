...
var _KnxParseTpci = _interopRequireDefault(require("./RawMod/KnxParseTpci"));
...
const KnxLog = require('./KnxLog'); // defaults


KnxProtocol.lengths = {}; // helper function: what is the byte length of an object?
...
KnxProtocol.define('IPv4Endpoint', {
  read: function (propertyName) {
    this.pushStack({
      addr: null,
      port: null
    }).raw('addr', 4).UInt16BE('port').tap(function (hdr) {
...
KnxProtocol.define('CRI', {
  read: function (propertyName) {
    ...
    .UInt8('header_length').UInt8('connection_type').UInt8('knx_layer').UInt8('unused').tap(function (hdr) {
      switch (hdr.connection_type) {
        ...
        default:
          throw Error('Unsupported connection type: ' + hdr.connection_type);
      }
    }).popStack(propertyName, function (data) {
      if (KnxProtocol.debug) KnxLog.get().debug('read CRI: ' + JSON.stringify(data)); // pop the interim value off the stack and insert the real value into `propertyName`

      return data;
    });
  },
  write: function (value) {
    if (!value) KnxLog.get().warn('CRI: cannot write null value for CRI');else {
      this.UInt8(0x04) // length
      .UInt8(value.connection_type).UInt8(value.knx_layer).UInt8(value.unused);
    }
  }
});
...
KnxProtocol.define('ConnState', {
  read: function (propertyName) {
    this.pushStack({
      channel_id: null,
      status: null
    }).UInt8('channel_id').UInt8('status').popStack(propertyName, function (data) {
...
KnxProtocol.define('TunnState', {
  read: function (propertyName) {
    this.pushStack({
      ...
      seqnum: null,
      rsvd: null
    }).UInt8('header_length').UInt8('channel_id').UInt8('seqnum').UInt8('rsvd').tap(function (hdr) {
  ...
  write: function (value) {
    ...
    this.UInt8(0x04).UInt8(value.channel_id).UInt8(value.seqnum).UInt8(value.rsvd);
  }
});
...
KnxProtocol.define('HPAI', {
  read: function (propertyName) {
    ...
    }).UInt8('header_length').UInt8('protocol_type').IPv4Endpoint('tunnel_endpoint').tap(function (hdr) {
  ...
  write: function (value) {
    ...
    .UInt8(value.protocol_type).IPv4Endpoint(value.tunnel_endpoint);
  }
});
...
let ctrlStruct = new Parser() // byte 1
.bit1('frameType').bit1('reserved').bit1('repeat').bit1('broadcast').bit2('priority').bit1('acknowledge').bit1('confirm') // byte 2
.bit1('destAddrType').bit3('hopCount').bit4('extendedFrame'); // APDU: 2 bytes, tcpi = 6 bits, apci = 4 bits, remaining 6 bits = data (when length=1)
...
KnxProtocol.define('APDU', {
  read: function (propertyName) {
    this.pushStack({
      ...
      apci: null,
      data: null
    }).UInt8('apdu_length').tap(function (hdr) {
      ...
      if (hdr.apdu_length === 0) {
        ...
        hdr.apci = _KnxParseTpci.default.parseTpci(hdr.tpci);
      } else {
        // Parse the raw apdu
        const apdu = KnxProtocol.apduStruct.parse(hdr.apdu_raw); // Get the tcpi value (Shift it to the right to make working with it easier)
        ...
        if (KnxConstants.keyText(KnxConstants.APCICODES, altApciCode) && altApciCode > KnxConstants.APCICODES.Memory_Read) {
          ...
          hdr.apci = KnxConstants.keyText(KnxConstants.APCICODES, apdu.apci * 0x40); // PhysicalAddress_Response messages do not carry any data at all
  ...
  write: function (value) {
    ...
    const totalLength = knxlen('APDU', value); // handle messages without the APCI field
    ...
    if (totalLength === 3 && !value.inApciData && !value.noData) {
      ...
      this.UInt16BE(word); // Check if there is explicitly not data
...
KnxProtocol.lengths['APDU'] = function (value) {
  if (!value) return 0; // Check if there is a APCI field
  ...
  if (!value.apci) {
    ...
    if (value.bitlength || value.data && value.data.bitlength) {
      const bitlen = value.bitlength || value.data.bitlength; // KNX spec states that up to 6 bits of payload must fit into the TPCI
    ...
    if (value.data.length) {
      ...
      if (!isNaN(parseFloat(value.data)) && isFinite(value.data) && value.data >= 0 && value.data <= 63) {
        return 3;
      } else {
...
KnxProtocol.define('CEMI', {
  read: function (propertyName) {
    this.pushStack({
      ...
      dest_addr: null,
      apdu: null
    }).UInt8('msgcode').UInt8('addinfo_length').raw('ctrl', 2).raw('src_addr', 2).raw('dest_addr', 2).tap(function (hdr) {
      // parse 16bit control field
      hdr.ctrl = ctrlStruct.parse(hdr.ctrl); // KNX source addresses are always physical
  ...
  write: function (value) {
    ...
    const validateDestStr = dest => {
      ...
      if (KnxAddress.validateAddrStr(dest) === -1) {
        ...
        this.error = Error(util.format('KNX destination address () should be a %s address (%j)!', value.ctrl.destAddrType === KnxConstants.KNX_ADDR_TYPES.DEVICE ? 'device' : 'group', dest));
        retVal = 1;
      }
    ...
    let ctrlField1 = value.ctrl.frameType * 0x80 + value.ctrl.reserved * 0x40 + value.ctrl.repeat * 0x20 + value.ctrl.broadcast * 0x10 + value.ctrl.priority * 0x04 + value.ctrl.acknowledge * 0x02 + value.ctrl.confirm;
    let ctrlField2 = value.ctrl.destAddrType * 0x80 + value.ctrl.hopCount * 0x10 + value.ctrl.extendedFrame;
    let srcAddr;
    let destAddr; // Check the type of value.src_addr and value.dest_addr an assume that they have the same type
    ...
    if (!(srcAddr !== null && destAddr !== null)) {
      this.error = Error(util.format('The destination (%j) or the source (%j) address is invalid!', value.dest_addr, value.src_addr));
      this.buffer = null;
      return;
    }

    this.UInt8(value.msgcode).UInt8(value.addinfo_length).UInt8(ctrlField1).UInt8(ctrlField2).raw(srcAddr, 2).raw(destAddr, 2); // only need to marshal an APDU if this is a
...
KnxProtocol.define('KNXNetHeader', {
  read: function (propertyName) {
    this.pushStack({
      ...
      service_type: -1,
      total_length: 0
    }).UInt8('header_length').UInt8('protocol_version').UInt16BE('service_type').UInt16BE('total_length').tap(function (hdr) {
      if (KnxProtocol.debug) KnxLog.get().trace('read KNXNetHeader :%j', hdr);

      if (this.buffer.length + hdr.header_length < this.total_length) {
        throw util.format('Incomplete KNXNet packet: got %d bytes (expected %d)', this.buffer.length + hdr.header_length, this.total_length);
      }
  ...
  write: function (value) {
     ...
     */
    this.error = null; // Check if value (KNX message) is defined
    ...
    .UInt16BE(value.service_type).UInt16BE(value.total_length);
...
KnxProtocol.lengths['KNXNetHeader'] = function (value) {
  ...
  switch (value.service_type) {
    // case SERVICE_TYPE.SEARCH_REQUEST:
    case KnxConstants.SERVICE_TYPE.CONNECT_REQUEST:
      return 6 + knxlen('HPAI', value.hpai) + knxlen('HPAI', value.tunn) + knxlen('CRI', value.cri);

    case KnxConstants.SERVICE_TYPE.CONNECT_RESPONSE:
    case KnxConstants.SERVICE_TYPE.CONNECTIONSTATE_REQUEST:
    case KnxConstants.SERVICE_TYPE.CONNECTIONSTATE_RESPONSE:
    case KnxConstants.SERVICE_TYPE.DISCONNECT_REQUEST:
      return 6 + knxlen('ConnState', value.connstate) + knxlen('HPAI', value.hpai) + knxlen('CRI', value.cri);

    case KnxConstants.SERVICE_TYPE.TUNNELING_ACK:
    case KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST:
      return 6 + knxlen('TunnState', value.tunnstate) + knxlen('CEMI', value.cemi);
