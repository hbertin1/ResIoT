...
const DEFAULT_READ_GROUPVAL_TIMEOUT = 3000; // bind incoming UDP packet handler

const onUdpSocketMessage = function (msg
/*, rinfo, callback */
) {
  ...
  try {
    ...
    if (dg) {
      ...
      if (!isNaN(this.channel_id) && (dg.hasOwnProperty('connstate') && dg.connstate.channel_id !== this.channel_id || dg.hasOwnProperty('tunnstate') && dg.tunnstate.channel_id !== this.channel_id)) {
        ...
        _Handlers.default.rawMsgHandler(msg, dg, this);
        /*******************************/
       ...
       */
      _Handlers.default.brokenMsgHandler(msg, this);
    }
    /*******************************/

  } catch (err) {
    console.trace(err);
    KnxLog.get().debug('(%s): Incomplete/unparseable UDP packet: %s: %s', this.compositeState(), err, msg.toString());
  }
};
...
const AddCEMI = function (datagram, msgcode) {
  ...
  datagram.cemi = {
    msgcode: msgcode || 0x11,
    // default: L_Data.req for tunneling
    ctrl: {
      frameType: 1,
      // 0=extended 1=standard
      reserved: 0,
      // always 0
      repeat: 1,
      // the OPPOSITE: 1=do NOT repeat
      broadcast: 1,
      // 0-system broadcast 1-broadcast
      priority: 3,
      // 0-system 1-normal 2-urgent 3-low
      acknowledge: sendAck ? 1 : 0,
      confirm: 0,
      // FIXME: only for L_Data.con 0-ok 1-error
      destAddrType: 1,
      // FIXME: 0-physical 1-groupaddr
      hopCount: 6,
      extendedFrame: 0
    },
...
const Request = function (type, datagramTemplate, callback) {
  const self = this; // populate skeleton datagram

  const datagram = this.prepareDatagram(type); // decorate the datagram, if a function is passed
  ...
  const st = KnxConstants.keyText(KnxConstants.SERVICE_TYPE, type); // hand off the outbound request to the state machine
...
const send = function (datagram, callback) {
  ...
  if (datagram.constructor !== Buffer) {
    this.writer = KnxNetProtocol.createWriter(); // Forge the datagram

    ret = this.writer.KNXNetHeader(datagram); // Check if ret.buffer is null ==> if this.writer.KNXNetHeader() failed

    if (ret.buffer === null) {
      // Check if error was set - if not, pass a standard message to callback
      ret.error !== null ? callback(ret.error) : callback(Error('Unknown error!')); // Cancel the sending process
  ...
  this.socket.send(buf, 0, buf.length, conn.remoteEndpoint.port, conn.remoteEndpoint.addr.toString(), function (err) {
    KnxLog.get().trace('(%s): UDP sent %s: %s %s', conn.compositeState(), err ? err.toString() : 'OK', descr, buf.toString());
...
const write = function (grpaddr, value, dptid, callback) {
  ...
  const serviceType = this.useTunneling ? KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST : KnxConstants.SERVICE_TYPE.ROUTING_INDICATION;
...
const respond = function (grpaddr, value, dptid) {
  ...
  const serviceType = this.useTunneling ? KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST : KnxConstants.SERVICE_TYPE.ROUTING_INDICATION;
  this.Request(serviceType, function (datagram) {
    DPTLib.populateAPDU(value, datagram.cemi.apdu, dptid); // this is a READ request
...
const read = function (grpaddr, callback) {
  if (typeof callback === 'function') {
    const conn = this; // Determine the timeout for receiving the response

    let timeout = this.readGroupValTimeout ? this.readGroupValTimeout : DEFAULT_READ_GROUPVAL_TIMEOUT; // when the response arrives:
    ...
    const binding = function (src, data) {
      ...
      conn.off(responseEvent, binding); // fire the callback

      callback(src, data);
    }; // prepare for the response


    this.on(responseEvent, binding); // clean up after 3 seconds just in case no one answers the read request
...
const Disconnect = function
  /* cb */
() {
  this.transition('disconnecting'); // machina.js removeAllListeners equivalent:
  // this.off();
...
const datagramDesc = function (dg) {
  ...
  if (dg.service_type === KnxConstants.SERVICE_TYPE.TUNNELING_REQUEST || dg.service_type === KnxConstants.SERVICE_TYPE.ROUTING_INDICATION) {
...
const Connection = function (options) {
  const conn = new _FSM.default(options); // Add functions from this file
  ...
  conn.AddTunn = AddTunn; // register with the FSM any event handlers passed into the options object
