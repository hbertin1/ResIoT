...
var _default = {
  ...
  readDevMem: async (target, source, address, length, recvTimeout, conContext, errContext) => {
    ...
    return new Promise(async resolve => {
      ...
      let dconnMsg;
      let timeoutRef;
      let memoryResponseHandlerTemplate; // This function validates the arguments
      ...
      const checkArguments = () => {
        ...
        const checkArgumentTypes = () => {
          if (target.constructor !== String || (source != null ? source.constructor !== String : false) || address.constructor !== Array || length.constructor !== Number || recvTimeout.constructor !== Number) {
        ...
        const checkLengthArgumentVal = () => {
          if (length > 12 || length < 1) {
            err = new Error(_Errors.default.INVALID_READLEN.errorMsg);
            rawModErr = errContext.createNewError(err, _Errors.default.INVALID_READLEN.errorID);
            errnum = errContext.addNewError(rawModErr);
            return 1;
          }
      ...
      const forgeMessages = () => {
        ...
        memReadReq = _KnxMessageTemplates.default.memoryReadRequest(target, source, address, length);
      ...
      const prepareCustomMessageHandlerTemplates = () => {
        memoryResponseHandlerTemplate = _CustomMessageHandlerTemplates.default.memoryResponseTemplate(target, source || conContext.options.physAddr);
      ...
      const registerHandler = () => {
         ...
         */
        _CustomMsgHandlers.default.registerCustomMsgHandler(memoryResponseHandlerTemplate, memoryResponseHandler, conContext);
      }; // This function is later used to remove handlers registered below


      const removeHandlers = () => {
        _CustomMsgHandlers.default.removeCustomMsgHandler(memoryResponseHandlerTemplate, conContext);
        ...
        return new Promise(resolve => {
          ...
          _KnxNetProtocol.default.sendTunnRequest(connReq, conContext, sendErr => {
            ...
            if (sendErr) {
              ...
              _KnxNetProtocol.default.sendTunnRequest(memReadReq, conContext, sendErr => {
                // Check for errors
                if (sendErr) {
                  // Create the RawModError object
                  rawModErr = errContext.createNewError(sendErr, null); // Push it onto the errorStack
