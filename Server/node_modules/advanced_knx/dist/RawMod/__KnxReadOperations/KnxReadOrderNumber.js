...
var _KnxReadDeviceResource = _interopRequireDefault(require("./KnxReadDeviceResource"));
...
var _default = {
   ...
   */
  readOrderNumber: async (target, recvTimeout, maskVersion, conContext, errContext) => {
    return new Promise(async resolve => {
      // read the resource
      const val = await _KnxReadDeviceResource.default.readDeviceResource(target, null, maskVersion, 'DeviceOrderId', _KnxConstants.default.RESOURCE_ACCESS_TYPES.ALL, recvTimeout, conContext, errContext);

      if (!val.error) {
        // Cut of the first n bytes - n depends on the access way used
        if (val.usedAccessMethod === _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY) {
          // Used memory address - cut off first three bytes
          val.orderNumber = val.data.slice(3);
        } else {
          // Used property access - cut off first four bytes
          val.orderNumber = val.data.slice(4);
        }
      } // Return the result
