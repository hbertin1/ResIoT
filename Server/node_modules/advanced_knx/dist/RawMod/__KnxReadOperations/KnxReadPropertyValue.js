...
var _default = {
  ...
  readPropertyValue: async (target, source, objectIndex, startIndex, propertyID, elementCount, recvTimeout, conContext, errContext) => {
    ...
    return new Promise(async resolve => {
      ...
      let dconnMsg;
      let timeoutRef;
      let propertyValueResponseHandlerTemplate; // This function validates the arguments
      ...
      const checkArguments = () => {
        ...
        const checkArgumentsDefined = () => {
          ...
          if (!(target && recvTimeout && conContext && propertyID && startIndex && elementCount)) {
            ...
            rawModErr = errContext.createNewError(err, _Errors.default.UNDEF_ARGS.errorID);
        ...
        const checkArgumentTypes = () => {
          if (target.constructor !== String || (source ? source.constructor !== String : false) || objectIndex.constructor !== Number || propertyID.constructor !== Number || startIndex.constructor !== Number || elementCount.constructor !== Number || recvTimeout.constructor !== Number) {
      ...
      const forgeMessages = () => {
        ...
        propValReadReq = _KnxMessageTemplates.default.propertyValueReadRequest(target, source, objectIndex, propertyID, elementCount, startIndex);
      ...
      const prepareCustomMessageHandlerTemplates = () => {
        propertyValueResponseHandlerTemplate = _CustomMessageHandlerTemplates.default.propertyValueResponseTemplate(target, source || conContext.options.physAddr);
      ...
      const propValResponseHandler = rawMsgJson => {
        ...
        _KnxNetProtocol.default.sendTunnRequest(ackMsg, conContext, function (sendErr) {
          ...
          _KnxNetProtocol.default.sendTunnRequest(dconnMsg, conContext, function (sendErr) {
            ...
            resolve({
              error: 0,
              data: rawMsgJson.cemi.apdu.data
            });
            /********************************/
          });
        });
      ...
      const registerHandler = () => {
         ...
         */
        _CustomMsgHandlers.default.registerCustomMsgHandler(propertyValueResponseHandlerTemplate, propValResponseHandler, conContext);
      }; // This function is later used to remove handlers registered below


      const removeHandlers = () => {
        _CustomMsgHandlers.default.removeCustomMsgHandler(propertyValueResponseHandlerTemplate, conContext);
      ...
      const sendConnAndReadReq = () => {
        return new Promise(resolve => {
          ...
          _KnxNetProtocol.default.sendTunnRequest(connReq, conContext, sendErr => {
            // Check for errors
            if (sendErr) {
              // Create the RawModError object
              rawModErr = errContext.createNewError(sendErr, null); // Push it onto the errorStack

              const errnum = errContext.addNewError(rawModErr); // Return 1

              resolve(errnum);
            } else {
              // Send the memory read request
              _KnxNetProtocol.default.sendTunnRequest(propValReadReq, conContext, sendErr => {
                // Check for errors
                if (sendErr) {
                  // Create the RawModError object
                  rawModErr = errContext.createNewError(sendErr, null); // Push it onto the errorStack
      ...
      let e; // Call all functions defined above
