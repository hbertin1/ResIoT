...
var _KnxDeviceResourceInformation = _interopRequireDefault(require("../KnxDeviceResourceInformation"));

var _KnxReadResourceViaMemory = _interopRequireDefault(require("./__KnxReadResourceViaMemory"));

var _KnxReadResourceViaProperty = _interopRequireDefault(require("./__KnxReadResourceViaProperty"));
...
var _default = {
  ...
  readDeviceResource: async (target, source, deviceMaskversion, resourceNameStr, preferredReadType, recvTimeout, conContext, errContext) => {
    return new Promise(async resolve => {
      ...
      let rawModErr;
      let err; // The value to be passed to resolve
      ...
      const getResourceInformation = () => {
        deviceResourceInformation = _KnxDeviceResourceInformation.default.getResourceInfoByMaskVersionAndName(deviceMaskversion, resourceNameStr); // Check if anything was found

        if (!deviceResourceInformation) {
          err = new Error(_Errors.default.INVALID_MV_RN.errorMsg);
          rawModErr = errContext.createNewError(err, _Errors.default.INVALID_MV_RN.errorID);
      ...
      const checkAvailableReadMethods = () => {
        ...
        if (!(readViaMemAccessAvailable || readViaPropertyAvailable)) {
          err = new Error(_Errors.default.NO_READ_WAY_FOUND.errorMsg);
          rawModErr = errContext.createNewError(err, _Errors.default.NO_READ_WAY_FOUND.errorID);
          retVal.error = errContext.addNewError(rawModErr);
          return 1;
        }
      ...
      const chooseReadMethodFunction = () => {
        if (readViaPropertyAvailable && readViaMemAccessAvailable) {
          ...
          if (preferredReadType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY) {
            readMethodFunction = _KnxReadResourceViaMemory.default;
          } else {
            readMethodFunction = _KnxReadResourceViaProperty.default;
          }
        } else if (!readViaMemAccessAvailable && preferredReadType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY_STRICT) {
          // Using direct memory access forced but not possible - error
          readMethodFunction = undefined;
        } else if (!readViaPropertyAvailable && preferredReadType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.PROPERTY_STRICT) {
        ...
        if (!readMethodFunction) {
          err = new Error(_Errors.default.NO_READ_WAY_MATCHED.errorMsg);
          rawModErr = errContext.createNewError(err, _Errors.default.NO_READ_WAY_MATCHED.errorID);
          retVal.error = errContext.addNewError(rawModErr);
          return 1;
        }
      ...
      if (getResourceInformation()) {
        resolve(retVal);
        return;
      }

      if (checkAvailableReadMethods()) {
        resolve(retVal);
        return;
      }

      if (chooseReadMethodFunction()) {
        resolve(retVal);
        return;
      }
      ...
      if (readMethodFunction === _KnxReadResourceViaMemory.default) {
        retVal.usedAccessMethod = _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY;
      } else {
        retVal.usedAccessMethod = _KnxConstants.default.RESOURCE_ACCESS_TYPES.PROPERTY;
