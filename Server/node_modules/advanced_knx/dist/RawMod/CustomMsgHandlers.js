...
var _KnxNetProtocolExtra = _interopRequireDefault(require("./KnxNetProtocolExtra"));
...
class RawModCustomMsgHandlers {
        ...
        for (let entry in conContext.handlers.customMsgHandlers) {
          ...
          if (conContext.handlers.customMsgHandlers.hasOwnProperty(entry)) {
            ...
            if (typeof conContext.handlers.customMsgHandlers[entry].callback === 'function') {
               ...
               */
              if (new _KnxNetProtocolExtra.default().messageMatchesTemplate(conContext.handlers.customMsgHandlers[entry].template, rawMsgJson) === 0) {
                /*
  ...
  static registerCustomMsgHandler(template, callback, conContext) {
    ...
    const lengthBefore = conContext.handlers.customMsgHandlers.length; // Add template and callback to the custom message handler array
  ...
  static removeCustomMsgHandler(template, conContext) {
    ...
    const lengthBefore = conContext.handlers.customMsgHandlers.length; // Find and delete all matching handlers
...
exports.default = RawModCustomMsgHandlers;