...
var _KnxWriteResourceViaMemory = _interopRequireDefault(require("./__KnxWriteResourceViaMemory"));

var _KnxWriteResourceViaProperty = _interopRequireDefault(require("./__KnxWriteResourceViaProperty"));
    ...
    return new Promise(async resolve => {
      ...
      const checkAvailableWriteMethods = () => {
        ...
        if (!(writeViaMemAccessAvailable || writeViaPropertyAvailable)) {
          err = new Error(_Errors.default.NO_WRITE_WAY_FOUND);
          rawModErr = errContext.createNewError(err, _Errors.default.NO_WRITE_WAY_FOUND.errorID);
          retVal = errContext.addNewError(rawModErr);
          return 1;
        }
      ...
      const chooseWriteMethodFunction = () => {
        if (writeViaPropertyAvailable && writeViaMemAccessAvailable) {
          ...
          if (preferredWriteType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY) {
            writeMethodFunction = _KnxWriteResourceViaMemory.default;
          } else {
            writeMethodFunction = _KnxWriteResourceViaProperty.default;
          }
        } else if (!writeViaMemAccessAvailable && preferredWriteType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.MEMORY_STRICT) {
          // Using direct memory access forced but not possible - error
          writeMethodFunction = undefined;
        } else if (!writeViaPropertyAvailable && preferredWriteType === _KnxConstants.default.RESOURCE_ACCESS_TYPES.PROPERTY_STRICT) {
        ...
        if (!writeMethodFunction) {
          err = new Error(_Errors.default.NO_WRITE_WAY_MATCHED.errorMsg);
          rawModErr = errContext.createNewError(err, _Errors.default.NO_WRITE_WAY_MATCHED.errorID);
          retVal = errContext.addNewError(rawModErr);
          return 1;
        }
      ...
      if (checkAvailableWriteMethods()) {
        resolve(retVal);
        return;
      }

      if (chooseWriteMethodFunction()) {
        resolve(retVal);
        return;
      }

      await readResource(); // Resolve the promise - retVal will be passed trough from the chosen read function
