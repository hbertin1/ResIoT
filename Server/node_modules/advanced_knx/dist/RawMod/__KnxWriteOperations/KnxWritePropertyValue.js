...
var _KnxMessageTemplates = _interopRequireDefault(require("../KnxMessageTemplates"));

var _KnxNetProtocol = _interopRequireDefault(require("../KnxNetProtocol"));

var _CustomMsgHandlers = _interopRequireDefault(require("../CustomMsgHandlers"));

var _CustomMessageHandlerTemplates = _interopRequireDefault(require("../CustomMessageHandlerTemplates"));

var _KnxAddress = _interopRequireDefault(require("../KnxAddress"));
...
var _default = {
  ...
  writePropertyValue: async (target, source, objectIndex, startIndex, propertyID, elementCount, data, recvTimeout, conContext, errContext) => {
    ...
    return new Promise(async resolve => {
      ...
      let ncdNackHandlerTemplate; // This function validates the arguments

      const checkArguments = () => {
        let errnum = 0; // This function checks if all the arguments are defined
        ...
        const checkArgumentsDefined = () => {
          ...
          if (!(target && recvTimeout && conContext && data)) {
            err = new Error(_Errors.default.UNDEF_ARGS.errorMsg);
            rawModErr = errContext.createNewError(err, _Errors.default.UNDEF_ARGS);
        ...
        const checkArgumentTypes = () => {
          if (target.constructor !== String || (source ? source.constructor !== String : false) || objectIndex.constructor !== Number || propertyID.constructor !== Number || data.constructor !== Buffer || startIndex.constructor !== Number || elementCount.constructor !== Number || recvTimeout.constructor !== Number) {
            err = new Error(_Errors.default.INVALID_ARGTYPES.errorMsg);
            rawModErr = errContext.createNewError(err, _Errors.default.INVALID_ARGTYPES.errorID);
            errnum = errContext.addNewError(rawModErr);
            return 1;
          }
        ...
        const checkDataVal = () => {
          if (data.length > 10 || data.length < 1) {
            err = new Error(_Errors.default.INVALID_DATALEN.errorMsg);
            rawModErr = errContext.createNewError(err, _Errors.default.INVALID_DATALEN.errorID);
            errnum = errContext.addNewError(rawModErr);
            return 1;
          }
        ...
        const checkTargetAndSource = () => {
          let retVal = 0; // Validate target

          if (_KnxAddress.default.validateAddrStr(target) === -1 || _KnxAddress.default.getAddrType(target) !== _KnxConstants.default.KNX_ADDR_TYPES.DEVICE) {
            err = new Error(_Errors.default.INVALID_TARGET.errorMsg);
            rawModErr = errContext.createNewError(err, _Errors.default.INVALID_TARGET.errorID);
            errnum = errContext.addNewError(rawModErr);
            retVal = 1;
          ...
          if (source) {
            if (_KnxAddress.default.validateAddrStr(source) === -1 || _KnxAddress.default.getAddrType(source) !== _KnxConstants.default.KNX_ADDR_TYPES.DEVICE) {
              err = new Error(_Errors.default.INVALID_SOURCE.errorMsg);
              rawModErr = errContext.createNewError(err, _Errors.default.INVALID_SOURCE.errorID);
              errnum = errContext.addNewError(rawModErr);
              retVal = 1;
            }
          } // Return
        ...
        if (checkArgumentsDefined()) {
          return errnum;
        }

        if (checkArgumentTypes()) {
          return errnum;
        }
      ...
      const forgeMessages = () => {
        connReq = _KnxMessageTemplates.default.ucdConnRequest(target, source);
        propValWriteReq = _KnxMessageTemplates.default.propertyValueWriteRequest(target, source, objectIndex, propertyID, elementCount, startIndex, data);
        ackMsg = _KnxMessageTemplates.default.ncdAckMsg(target, source);
        dconnMsg = _KnxMessageTemplates.default.ucdDconnMsg(target, source);
      }; // This function prepares the custom message handler templates


      const prepareCustomMessageHandlerTemplates = () => {
        ncdAckHandlerTemplate = _CustomMessageHandlerTemplates.default.ncdAckHandlerTemplate(target, source || conContext.options.physAddr);
        ncdNackHandlerTemplate = _CustomMessageHandlerTemplates.default.ncdNackHandlerTemplate(target, source || conContext.options.physAddr);
      ...
      const ncdAckMsgHandler = () => {
        ...
        timeoutRef = null;
        removeHandlers(); // Send a NCD acknowledge message to the device

        _KnxNetProtocol.default.sendTunnRequest(ackMsg, conContext, function (sendErr) {
          if (sendErr) {
             ...
             */
            _KnxNetProtocol.default.sendTunnRequest(dconnMsg, conContext, function (sendErr) {
              if (sendErr) {
                rawModErr = errContext.createNewError(sendErr, null);
                const errnum = errContext.addNewError(rawModErr);
                resolve(errnum);
      ...
      const ncdNackMsgHandler = () => {
        ...
        clearTimeout(timeoutRef);
        timeoutRef = null;
        removeHandlers(); // Send the UCD disconnect message
      ...
      const registerHandlers = () => {
        _CustomMsgHandlers.default.registerCustomMsgHandler(ncdAckHandlerTemplate, ncdAckMsgHandler, conContext);

        _CustomMsgHandlers.default.registerCustomMsgHandler(ncdNackHandlerTemplate, ncdNackMsgHandler, conContext);
      }; // This function is later used to remove handlers registered below


      const removeHandlers = () => {
        _CustomMsgHandlers.default.removeCustomMsgHandler(ncdAckHandlerTemplate, conContext);

        _CustomMsgHandlers.default.removeCustomMsgHandler(ncdNackHandlerTemplate, conContext);
      ...
      const sendConnAndWriteReq = () => {
        return new Promise(resolve => {
          ...
          _KnxNetProtocol.default.sendTunnRequest(connReq, conContext, function (sendErr) {
            if (sendErr) {
              // Create the RawModError object
              rawModErr = errContext.createNewError(sendErr, null); // Push it onto the errorStack

              const errnum = errContext.addNewError(rawModErr); // Return errnum

              resolve(errnum);
            } else {
              // Send the memory write request
              _KnxNetProtocol.default.sendTunnRequest(propValWriteReq, conContext, function (sendErr) {
                if (sendErr) {
                  // Create the RawModError object
                  rawModErr = errContext.createNewError(sendErr, null); // Push it onto the errorStack
      ...
      const createRecvTimeout = () => {
        ...
        timeoutRef = setTimeout(function () {
          // Remove handlers
          removeHandlers(); // Create a Error() object

          err = new Error(_Errors.default.TIMEOUT_REACHED.errorMsg); // Create a RawModError object

          rawModErr = errContext.createNewError(err, _Errors.default.TIMEOUT_REACHED.errorID); // Push it onto the errorStack
      ...
      let e; // Call the functions defined above
