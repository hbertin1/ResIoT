...
class KnxAddress {
  ...
  static getAddrType(addrStr) {
    if (addrStr.lastIndexOf('/') !== -1) {
      return _KnxConstants.default.KNX_ADDR_TYPES.GROUP;
    } else if (addrStr.lastIndexOf('.') !== -1) {
      return _KnxConstants.default.KNX_ADDR_TYPES.DEVICE;
    } else {
      return -1;
    }
  }
  ...
  static validateAddrStr(addrStr) {
    let retVal = 0; // Check if addrStr is defined and at least three chars long (Shortest address: 0/0, 1/1, ...)
    ...
    if (!addrStr || addrStr.length < 3) {
      ...
      const deviceHint = addrStr.lastIndexOf('.'); // It has to be group OR device

      if (groupHint !== -1 && deviceHint !== -1 || groupHint === -1 && deviceHint === -1) {
        retVal = -1;
      } else {
        // If it is group, check the level - If it is device, check that it has exactly two separators
        if (groupHint !== -1) {
          const lvl = KnxAddress.grpAddrStrGetL(addrStr); // Check if it is a valid level
  ...
  static strToBin(addressStr) {
    let addrBin;
    let addrStrParts; // Check if addressStr is defined and if it has a .lastIndexOf() function
    ...
    if (addressStr.lastIndexOf('/') !== -1) {
      ...
      addrStrParts = addressStr.split('/'); // One separator: two parts; Two separators: three parts
      ...
      addrStrParts = addressStr.split('.'); // The first four bits are part one, the second four part two and the last eight part three
  ...
  static binToStrGrpL2(addressBin) {
    let addrStrP1;
    let addrStrP2; // Check if addressBin is defined and if it is valid (INT n; n >= 0 and n < 2**16) and if it has a .toString() function
    ...
    addrStrP1 = (addressBin >> 11).toString(); // Extract the lower eleven bits

    addrStrP2 = (addressBin & 0b0000011111111111).toString(); // Return the final value
  ...
  static binToStrGrpL3(addressBin) {
    let addrStrP1;
    let addrStrP2;
    let addrStrP3; // Check if addressBin is defined and if it is valid (INT n; n >= 0 and n < 2**16) and if it has a .toString() function
    ...
    addrStrP1 = (addressBin >> 11).toString(); // Extract the middle three bits

    addrStrP2 = ((addressBin & 0b0000011100000000) >> 8).toString(); // Extract the lower eight bits

    addrStrP3 = (addressBin & 0b0000000011111111).toString(); // Return the final value
  ...
  static binToInd(addressBin) {
    ...
    addrStrP1 = (addressBin >> 12).toString(); // Extract the middle four bits

    addrStrP2 = ((addressBin & 0b0000111100000000) >> 8).toString(); // Extract the lower eight bits
