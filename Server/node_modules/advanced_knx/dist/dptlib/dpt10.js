...
let timeRegexp = /(\d{1,2}):(\d{1,2}):(\d{1,2})/; // DPTFrame to parse a DPT10 frame.
...
exports.fromBuffer = function (buf) {
  if (buf.length !== 3) log.warn('DPT10: Buffer should be 3 bytes long');else {
    let d = new Date(); // FIXME: no ability to setDay() without week context
    ...
    if (hours >= 0 && hours <= 23 && minutes >= 0 && minutes <= 59 && seconds >= 0 && seconds <= 59) {
      ...
      log.warn('DPT10: buffer %j (decoded as %d:%d:%d) is not a valid time', buf, hours, minutes, seconds);
    }
