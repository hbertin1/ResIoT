...
exports.formatAPDU = function (value) {
  ...
  if (!isFinite(value)) {
    ...
    let mantissa = arr[0];
    let exponent = arr[1];
    let e; // find the minimum exponent that will upsize the normalized mantissa (0,5 to 1 range)
    // in order to fit in 11 bits ([-2048, 2047])

    let maxMantissa = 0;

    for (e = exponent; e >= -15; e--) {
      maxMantissa = ldexp(100 * mantissa, e);
      if (maxMantissa > -2048 && maxMantissa < 2047) break;
    }

    let sign = mantissa < 0 ? 1 : 0;
    let mant = mantissa < 0 ? ~(maxMantissa ^ 2047) : maxMantissa;
    let exp = exponent - e; // yucks

    apduData[0] = (sign << 7) + (exp << 3) + (mant >> 8);
    apduData[1] = mant % 256;
  }
...
exports.fromBuffer = function (buf) {
  if (buf.length !== 2) {
    ...
    let sign = buf[0] >> 7;
    let exponent = (buf[0] & 0b01111000) >> 3;
