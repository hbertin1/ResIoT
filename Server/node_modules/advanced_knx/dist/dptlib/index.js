...
for (var i = 0; i < dirEntries.length; i++) {
  if (matches = dirEntries[i].match(/(dpt.*)\.js/)) {
    ...
    if (!mod.hasOwnProperty('basetype') || !mod.basetype.hasOwnProperty('bitlength')) {
      throw 'incomplete ' + dptid + ', missing basetype and/or bitlength!';
    }

    mod.id = dptid;
    dpts[dptid] = mod; //log.trace('DPT library: loaded %s (%s)', dptid, dpts[dptid].basetype.desc);
  }
...
dpts.resolve = function (dptid) {
  ...
  if (m === null) {
    throw "Invalid DPT format: " + dptid;
  }

  var dpt = dpts[util.format('DPT%s', m[1])];

  if (!dpt) {
    throw "Unsupported DPT: " + dptid;
  }
...
dpts.populateAPDU = function (value, apdu, dptid) {
  ...
  var tgtvalue = value; // get the raw APDU data for the given JS value
  ...
  if (typeof dpt.formatAPDU == 'function') {
    ...
    apdu.data = dpt.formatAPDU(value); // log.trace('<<< custom formatAPDU(%s): %j', dptid, apdu.data);
  } else {
    if (!isFinite(value)) throw util.format("Invalid value, expected a %s", dpt.desc); // check if value is in range, be it explicitly defined or implied from bitlength

    var range = dpt.basetype.hasOwnProperty('range') ? dpt.basetype.range : [0, Math.pow(2, dpt.basetype.bitlength) - 1]; // is there a scalar range? eg. DPT5.003 angle degrees (0=0, ff=360)

    if (dpt.hasOwnProperty('subtype') && dpt.subtype.hasOwnProperty('scalar_range')) {
      var scalar = dpt.subtype.scalar_range;

      if (value < scalar[0] || value > scalar[1]) {
        log.trace("Value %j(%s) out of scalar range(%j) for %s", value, typeof value, scalar, dpt.id);
      } else {
      ...
      if (value < range[0] || value > range[1]) {
        log.trace("Value %j(%s) out of bounds(%j) for %s.%s", value, typeof value, range, dpt.id, dpt.subtypeid);
      }
...
dpts.fromBuffer = function (buf, dpt) {
  ...
  var value = 0; // get the raw APDU data for the given JS value
  ...
  if (typeof dpt.fromBuffer == 'function') {
    ...
    if (dpt.hasOwnProperty('subtype') && dpt.subtype.hasOwnProperty('scalar_range')) {
      var range = dpt.basetype.hasOwnProperty('range') ? dpt.basetype.range : [0, Math.pow(2, dpt.basetype.bitlength) - 1];
      var scalar = dpt.subtype.scalar_range; // convert value from its scalar representation
      // e.g. in DPT5.001, 50(%) => 0x7F , 100(%) => 0xFF

      var a = (scalar[1] - scalar[0]) / (range[1] - range[0]);
      var b = scalar[0] - range[0];
      value = Math.round(a * value + b); //log.trace('fromBuffer scalar a=%j b=%j %j', a,b, value);
    }
