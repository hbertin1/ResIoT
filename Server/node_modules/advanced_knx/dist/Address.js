...
let twoLevel = new Parser().bit5('l1').bit11('l2'); // convert address stored in two-byte buffer to string

Address.toString = function (buf
/* buffer */
, addrtype
/* ADDRESS_TYPE */
, twoLevelAddressing) {
  let group = addrtype === KnxConstants.KNX_ADDR_TYPES.GROUP;
  let address = null; // KnxLog.get().trace('%j, type: %d, %j', buf, addrtype, knxnetprotocol.twoLevelAddressing);
...
, twoLevelAddressing) {
  ...
  let tokens = addr.split(group ? '/' : '.').filter(w => {
    return w.length > 0;
  });
