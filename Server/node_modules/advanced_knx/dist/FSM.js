...
var _Handlers = _interopRequireDefault(require("./RawMod/Handlers"));
...
const states = {
  uninitialized: {
    '*': function () {
      ...
      this.isConnected = false; // try to connect
  ...
  connecting: {
    _onEnter: function () {
      ...
      let autoReconnect = this.options.hasOwnProperty('autoReconnect') ? this.options.autoReconnect : AUTO_RECONNECT_DEFAULT; // Check if the user called to disconnect
      ...
      if (this.useTunneling) {
        ...
        this.log.debug(util.format('Connecting via %s...', sm.localAddress)); // we retry 3 times, then restart the whole cycle using a slower and slower rate (max delay is 5 minutes)

        this.connecttimer = setInterval(function () {
          sm.connection_attempts += 1;

          if (sm.connection_attempts >= 1) {
            clearInterval(sm.connecttimer); // quite a few KNXnet/IP devices drop any tunneling packets received via multicast
            ...
            if (sm.remoteEndpoint.addr.range() === 'multicast') {
               ...
               */
              _Handlers.default.connFailHandler(this);
              /*
              ...
              if (autoReconnect) {
                 ...
                 */
                this.log.warn('connection timed out ... reconnecting'); // restart connecting cycle (cannot jump straight to 'connecting' so we use an intermediate state)
        ...
        delete this.lastSentTime; // send connect request directly
    ...
    inbound_CONNECT_RESPONSE: function (datagram) {
      ...
      if (datagram.hasOwnProperty('connstate') && datagram.connstate.status === KnxConstants.RESPONSECODE.E_NO_MORE_CONNECTIONS) {
        ...
        _Handlers.default.outOfConnectionsHandler(this);
        ...
        this.channel_id = datagram.connstate.channel_id; // Clear timeout for connection state requests to prevent it from sending requests with old eq. invalid data

        clearInterval(this.idletimer); // set the global knx address

        this.options.physAddr = Address.toString(Buffer.from([datagram.cri.knx_layer, datagram.cri.unused])); // send connectionstate request directly
        ...
        this.send(this.prepareDatagram(KnxConstants.SERVICE_TYPE.CONNECTIONSTATE_REQUEST), err => {
           ...
           */
          if (err) {
            _Handlers.default.sendFailHandler(err, this);
          }
        });
      }
    },
    inbound_CONNECTIONSTATE_RESPONSE: function (datagram) {
      if (this.useTunneling) {
        ...
        this.log.debug(util.format('Got connection state response, connstate: %s, channel ID: %d', str, datagram.connstate.channel_id));
        this.transition('connected');
      }
    },
    '*': function (data) {
  ...
  connected: {
    _onEnter: function () {
      ...
      this.reconnection_cycles = 0; // Reset outgoing sequence counter..

      this.seqnum = -1; // Set this.isConnected
  ...
  disconnecting: {
    ...
    inbound_DISCONNECT_RESPONSE: function
      /* datagram */
    () {
      if (this.useTunneling) {
  ...
  idle: {
    _onEnter: function () {
      ...
      KnxLog.get().debug('(%s):\t%s', this.compositeState(), ' zzzz...'); // process any deferred items from the FSM internal queue
    ...
    outbound_ROUTING_INDICATION: function (datagram, cb) {
      let sm = this;
      let elapsed = Date.now() - this.lastSentTime; // Get the correct minimumDelay value

      let mDelay;
      this.options.minimumDelay ? mDelay = this.options.minimumDelay : mDelay = DEFAULT_MINIMUM_DELAY; // if no minimum delay set OR the last sent datagram was long ago...
    ...
    outbound_TUNNELING_REQUEST: function (datagram, cb) {
      let sm = this; // Get the correct minimumDelay value

      let mDelay;
      this.options.minimumDelay ? mDelay = this.options.minimumDelay : mDelay = DEFAULT_MINIMUM_DELAY;

      if (this.useTunneling) {
        let elapsed = Date.now() - this.lastSentTime; // Check if mDelay ms passed and if there is a acknowledge message to be waited for and if the currentlySending flag is false
    ...
    inbound_DISCONNECT_REQUEST: function
      /* datagram */
    () {
      if (this.useTunneling) {
        this.transition('connecting');
  ...
  requestingConnState: {
    ...
    inbound_CONNECTIONSTATE_RESPONSE: function (datagram) {
      ...
      if (datagram.connstate.status === 0) {
        ...
        this.log.debug(util.format('*** error: %s *** (connstate.code: %d)', state, datagram.connstate.status));
        this.transition('connecting');
  ...
  sendDatagram: {
    _onEnter: function (datagram, cb) {
      const sm = this; // Switch the currentlySending flag to true

      this.currentlySending = true; // send the telegram on the wire
      ...
      this.send(datagram, err => {
        ...
        if (err) {
          // Report the failure
          _Handlers.default.sendFailHandler(err, sm); // Revert the change applied to this.seqnum because no message was sent


          this.seqnum--; // Go into the idle state

          this.transition('idle');
        } else {
          // Append the message to the array of sent messages
          if (sm.useTunneling) sm.sentTunnRequests[datagram.cemi.dest_addr] = datagram; // Set this.lastSentTime
        ...
        sm.currentlySending = false; // Call cb
  ...
  sendTunnReq_waitACK: {
    ...
    _onEnter: function (datagram) {
      const sm = this; // Signalize that a acknowledge message is waited

      this.waitingForAck = true; // this.log.debug('setting up tunnreq timeout for %j', datagram);
      ...
      this.tunnelingAckTimer = setTimeout(() => {
         ...
         */
        _Handlers.default.waitAckTimeoutHandler(this);
...
const acknowledge = function (datagram) {
  let ack = this.prepareDatagram(KnxConstants.SERVICE_TYPE.TUNNELING_ACK, datagram); // copy the sequence number and acknowledge
...
const emitEvent = function (datagram) {
  ...
  let evtName = datagram.cemi.apdu.apci; // 1. 'event_<dest_addr>', ''GroupValue_Write', src, data

  this.emit(util.format('event_%s', datagram.cemi.dest_addr), evtName, datagram.cemi.src_addr, datagram.cemi.apdu.data); // 2. 'GroupValue_Write_1/2/3', src, data

  this.emit(util.format('%s_%s', evtName, datagram.cemi.dest_addr), datagram.cemi.src_addr, datagram.cemi.apdu.data); // 3. 'GroupValue_Write', src, dest, data

  this.emit(evtName, datagram.cemi.src_addr, datagram.cemi.dest_addr, datagram.cemi.apdu.data); // 4. 'event', 'GroupValue_Write', src, dest, data
  ...
  for (let iface in interfaces) {
    for (let key in interfaces[iface]) {
      ...
      if (intf.family === 'IPv4' && !intf.internal) {
        this.log.trace(util.format('candidate interface: %s (%j)', iface, intf));
...
const getLocalAddress = function () {
  let candidateInterfaces = this.getIPv4Interfaces(); // if user has declared a desired interface then use it
...
var _default = machina.Fsm.extend({
