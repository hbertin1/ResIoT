...
function IpRoutingConnection(instance) {
  ...
  instance.BindSocket = function (cb) {
    let conn = this;
    let udpSocket = dgram.createSocket({
      type: 'udp4',
      reuseAddr: true
    });
    udpSocket.on('listening', function () {
      log.debug(util.format('IpRoutingConnection %s:%d, adding membership for %s', instance.localAddress, udpSocket.address().port, conn.remoteEndpoint.addr));
